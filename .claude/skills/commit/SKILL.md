...
4. 각 변경 파일의 경로를 기준으로 scope(client/server/shared/root/sync) 판별
5. type + scope 조합으로 커밋 분리 여부 판단
6. 커밋 메시지 초안 작성 후 사용자에게 확인
7. 승인 시 `git add` + `git commit` 실행
...
**scope는 반드시 포함해야 한다. scope 없는 커밋 메시지는 허용하지 않는다.**

변경된 파일의 경로를 기준으로 scope를 결정한다:

| 변경 파일 경로 | scope | 예시 |
|---------------|-------|------|
| `client/` 하위만 | `client` | `fix(client): 로그인 폼 유효성 검사 수정` |
| `server/` 하위만 | `server` | `feat(server): 사용자 프로필 조회 API 추가` |
| `shared/` 하위만 | `shared` | `chore(shared): auth 타입 추가` |
| 루트 파일만 (`.env`, `docker-compose.yml` 등) | `root` | `chore(root): docker-compose 포트 변경` |
| client + server 밀접하게 연결된 변경 | `sync` | `feat(sync): 회원가입 API 연동` |
| 여러 영역에 걸친 설정/인프라 변경 | `root` | `chore(root): shared 패키지 세팅` |

scope 판별 순서:
1. 변경 파일 목록에서 각 파일의 최상위 디렉토리(client/server/shared/루트)를 확인
2. 한 영역의 파일만 있으면 → 해당 scope
3. 여러 영역이 섞여 있으면 → 독립적 변경이면 scope별로 커밋 분리, 밀접한 변경이면 `sync` 또는 `root`
...
style(client): import 정렬 및 미사용 변수 제거

chore(root): shared 패키지 세팅

- pnpm-workspace에 shared 추가
- client/server에 shared 의존성 및 paths 설정
