...
├── title (템플릿 이름)
├── description (설명)
├── data (JSON - TemplateData: canvasConfig + elements 배열)
...
#### 2-2. 템플릿 data JSON 구조 (TemplateData)

DB에는 `data` 컬럼에 `TemplateData` 객체를 JSON으로 저장한다.
...
```json
{
  ...
  "elements": [
    ...
    },
    {
      ...
      "zIndex": 2,
      "locked": false,
      "content": { "type": "doc", "content": [{ "type": "heading", "attrs": { "level": 1 }, "content": [{ "type": "text", "text": "홍길동" }] }, { "type": "paragraph", "content": [{ "type": "text", "text": "프론트엔드 개발자" }] }] },
...
요소 타입:
- `rect` — 사각형 도형 (배경, 구분 영역). fill, stroke, borderRadius, opacity 속성
- `line` — 구분선. stroke, strokeWidth, opacity 속성
- `markdown` — tiptap 리치 텍스트 블록. `editable: true`이면 이력서 작성 시 편집 가능. content는 tiptap JSON 형식

모든 요소는 공통 속성(id, type, x, y, width, height, zIndex, locked)을 가진다.

#### 2-3. 노코드 빌더 UI

- `/builder` — 새 템플릿 제작
- `/builder/[id]` — 기존 템플릿 편집
- **A4 고정 캔버스** (794 × 1123px, canvasConfig으로 관리)
- 좌측 사이드바: 요소 팔레트 (사각형, 선, 마크다운 텍스트 블록 추가)
...
|--------|----------|------|------|
| POST | /templates | JWT | 템플릿 생성 |
| GET | /templates/my | JWT | 내 템플릿 목록 (DRAFT 포함) |
| GET | /templates/:id | - | 템플릿 상세 조회 |
| PATCH | /templates/:id | Owner | 템플릿 수정 |
| DELETE | /templates/:id | Owner | 템플릿 삭제 |
| PATCH | /templates/:id/publish | Owner | 발행/비공개 전환 |
| POST | /templates/:id/fork | JWT | 템플릿 복제 (내 소유로 사본 생성) |

- 본인 소유 템플릿만 수정/삭제 가능 (Authorization Guard)
- fork 시 data JSON(canvasConfig + elements)을 복사하여 새 Template 레코드 생성, forkedFromId에 원본 ID 기록
...
| Method | Endpoint | Auth | 설명 |
|--------|----------|------|------|
| POST | /resumes | JWT | 이력서 생성 (templateId 필수) |
| GET | /resumes/my | JWT | 내 이력서 목록 (DRAFT 포함) |
| GET | /resumes/:id | JWT/Owner | 이력서 상세 조회 (본인: MyResumeResponse, 타인: ResumeDetailResponse) |
| PATCH | /resumes/:id | Owner | 이력서 수정 (content, thumbnail 등) |
| DELETE | /resumes/:id | Owner | 이력서 삭제 |
| PATCH | /resumes/:id/publish | Owner | 발행/비공개 전환 (status, pricingType, price) |

- 본인 소유 이력서만 수정/삭제 가능
- 이력서 조회 시 연결된 Template의 data(canvasConfig + elements)를 templateData로 함께 반환 (렌더링용)
...
| Method | Endpoint | Auth | 설명 |
|--------|----------|------|------|
| GET | /templates | - | 공개 템플릿 목록 (필터/정렬/페이지네이션) |
| GET | /templates/:id | - | 템플릿 상세 조회 |
| GET | /explore | - | 공개 이력서 목록 (필터/정렬/페이지네이션) |
| GET | /r/:slug | - | 이력서 열람 (공개 URL) |
...
├── type (VIEW_REWARD | PURCHASE_EARNING | POINT_USAGE)
├── amount (양수: 적립, 음수: 차감)
├── balance (거래 후 잔액)
├── description (설명)
├── referenceType (TEMPLATE | RESUME, nullable)
├── referenceId (관련 콘텐츠 ID, nullable)
├── createdAt
```
...
```
유료 콘텐츠 구매:
  1. 결제 금액 = 콘텐츠 가격 - 포인트 사용액 (보유 잔액 범위 내)
  2. 원화 결제 (PG사 미정) → 결제 성공
  3. Purchase 레코드 생성 (amount: 실결제 금액, pointsUsed: 포인트 사용액)
  4. 판매자 balance += 콘텐츠 가격의 90%
  5. 구매자 balance -= 사용한 포인트
...
무료 ↔ 유료 전환:
  PATCH /templates/:id/publish 또는 PATCH /resumes/:id/publish 로 pricingType, price 변경
...
| Method | Endpoint | Auth | 설명 |
|--------|----------|------|------|
| POST | /purchases | JWT | 구매 (targetType + targetId + usePoints?) |
| GET | /purchases/my | JWT | 내 구매 내역 |
| GET | /purchases/check/:targetType/:targetId | JWT | 구매 여부 확인 |
| GET | /rewards | JWT | 보상 내역 (RewardLog 목록, 페이지네이션) |
| GET | /rewards/summary | JWT | 보상 요약 (잔액, 총 적립, 총 사용) |

- 잔액 조회는 `GET /users/me` 응답(MyProfileResponse)의 `balance` 필드로 확인
...
├── rating (1~5 점수, 필수)
├── fieldKey (특정 필드에 대한 피드백, 선택 — Template의 fieldKey 대응)
├── createdAt, updatedAt
```
...
| Method | Endpoint | Auth | 설명 |
|--------|----------|------|------|
| POST | /resumes/:id/feedbacks | JWT | 피드백 작성 |
| GET | /resumes/:id/feedbacks | Owner | 이력서별 피드백 목록 (이력서 소유자만 조회 가능) |
| PATCH | /feedbacks/:id | 작성자 | 피드백 수정 |
| DELETE | /feedbacks/:id | 작성자 | 피드백 삭제 |
...
| Method | Endpoint | Auth | 설명 |
|--------|----------|------|------|
| POST | /reviews | JWT | 리뷰 작성 (targetType + targetId + rating + content) |
| PATCH | /reviews/:id | 작성자 | 리뷰 수정 (rating + content) |
| GET | /reviews | - | 대상별 리뷰 목록 (targetType + targetId, 페이지네이션) |
| GET | /reviews/summary/:targetType/:targetId | - | 평점 요약 (평균, 분포) |
...
├── type (FEEDBACK_RECEIVED | PURCHASE_RECEIVED | REWARD_EARNED | REVIEW_RECEIVED)
├── message (알림 메시지)
├── referenceType (TEMPLATE | RESUME | SELLER, nullable)
├── referenceId (관련 엔티티 ID, nullable)
...
| Method | Endpoint | Auth | 설명 |
|--------|----------|------|------|
| GET | /users/me | JWT | 내 프로필 조회 (balance 포함) |
| PATCH | /users/me | JWT | 프로필 수정 (name, bio, profileImage) |
| GET | /users/:id | - | 공개 프로필 조회 (avgRating, reviewCount 포함) |
| PATCH | /users/me/password | JWT | 비밀번호 변경 |
| DELETE | /users/me | JWT | 회원 탈퇴 |
...
- 포인트 할인: 보유 잔액(balance)으로 원화 결제 금액 차감 가능
- 출금 기능은 추후 구현
