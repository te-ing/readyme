...
- A4 고정 캔버스에서 도형, 선, 마크다운 텍스트를 자유롭게 배치
- 텍스트 블록에 `editable` 속성으로 이력서 작성 시 편집 가능 영역 지정
- 무료/유료로 공유 가능
...
- **노코드 템플릿 빌더**: react-rnd + tiptap 기반 A4 고정 캔버스에서 도형, 선, 마크다운으로 이력서 레이아웃 직접 제작
...
- **열람 보상**: 템플릿/이력서 조회수 기반 포인트 적립, 유료 판매 수익의 90% 적립
...
| 5     | 유료 구매 + 보상 (원화 결제, 포인트 할인, 보상 적립) | 미구현                        |
...
| Template     | 이력서 템플릿 (elements JSON, pricingType, viewCount, avgRating) |
